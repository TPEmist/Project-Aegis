...
- **Blob format (F13 v1)**: `MAGIC(2)=0x5050 ("PP") || VERSION(1)=0x01 || RESERVED(1)=0x00 || nonce(12) || ciphertext || tag(16)` (AES-256-GCM). The 4-byte header is bound into the AEAD Additional Authenticated Data (`setAAD` / `associated_data=`) so any tamper fails tag verification. Legacy pre-F13 blobs (no header) are still readable: the reader dispatches by magic, falls back to `nonce(12) || ciphertext || tag(16)` without AAD, and emits a one-time migration notice — the next `save_vault` rewrites as v1. Legacy reader will be removed in a future minor release (≥ 2 minor versions notice). See `pop_pay/vault.py` (`encrypt_credentials` / `decrypt_credentials`).
